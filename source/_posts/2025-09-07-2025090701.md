...
Safari では [v18.2 ですでに実装済み][safari]、Edge でも同じく [v140 でこの機能がサポートされました][edge][^2]が、本日はその実践的なユースケースとして「単位のない `line-height` 値を要素のフォントサイズに応じて動的に算出する」というのを Pure CSS でシンプルに書いてみたいと思います。
...
/* CSS Nesting is now Baseline 2023 */
  @media (width >= 48em) {
    font-size: 250%;
  }
}
...
<p class="codepen" data-height="640" data-default-tab="result,css" data-slug-hash="zxvyyQx" data-pen-title="フォントサイズだけ拡大した h1 要素のサンプル" data-user="jforg" style="height: 300px; box-sizing: border-box; display: flex; align-items: center; justify-content: center; border: 2px solid; margin: 1em 0; padding: 1em;">
...
その調整方法のひとつとして、現在このサイトで採用しているのが「見出しの `line-height` を常にその文字サイズ + ベースとなる本文のレディング長にする」というものです。ウチを例にして具体的に説明すると、
...
このコードの良いところは、もちろん Pure CSS なので都度コンパイルなどをしなくていいというのもありますが、何より**メディアクエリーなどを使ってレスポンシブに文字サイズを変更しても `line-height` は設定し直さなくていい**ところですね。`1em` は常にその要素自身の文字サイズを表しますから、文字サイズが変わればブラウザが再計算してくれるはずなので。
...
「それやったらお前、ピクセルの絶対値から比率を示す単位なしの数値に変換したったらええんちゃうんけ!」ということで、先ほどのコードを修正してこういう計算をした…くなりませんか?
...
なので、今までは単位なしの `line-height` 値を自動で算出するためには、これまで同様に Sass などの関数を使うか、もしくは単位なしの値が入ったカスタム・プロパティをいくつか用意しておいて、それを使って計算するくらいしか方法はなかった訳です。
...
[Chrome のリリースノート][chrome]でも少し触れられていますが、この他にも例えば、いわゆる Modular Scale の *** に使えそう（←今は伏せておく、後日書くかも）とか、タイポグラフィに応用する方法はいくつか思いつきました。詳しい方ならもっと色々と便利な手法が出てくるのかもしれません。
