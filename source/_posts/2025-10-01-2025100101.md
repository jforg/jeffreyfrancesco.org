...
もう少し具体的にいうと、プロジェクトの作業ディレクトリに `cd` コマンドで移動した際に <i class="filename">.node-versions</i> や <i class="filename">.nvmrc</i> といったファイルがあり[^2]、そこに指定されている Node.js バージョンがインストール済みであれば使用するランタイムがそのバージョンに切り替えられますし、未インストールであれば確認メッセージが表示されるので <kbd>y</kbd> を押すことでインストールされ、使えるようになります。
